...
import { sendSignalforMaskResponse, setMaskAreaData } from "../../../store/Slices/AdditionalDetailsSlice/addDetailsSlice";
...
// };
  ...
  const sendMaskData = async () => {
    ...
    try {
      ...
      const eventSource = new EventSource("https://8z2h0gbj-5001.inc1.devtunnels.ms/machine_learning_stream", {
        withCredentials: true,
      });
      dispatch(sendSignalforMaskResponse(true))
      eventSource.onmessage = (event) => {
        try {
          ...
          if (chunk.status === "Completed") {
            const {area} = chunk
            dispatch(setMaskAreaData(Object.values(chunk.area)))
            eventSource.close();
  ...
  const handleClick = () => {
    console.log("SegmentButton clicked!");
    setLoading(true)
    sendMaskData();
  };
